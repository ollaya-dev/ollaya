...
Execute tasks one at a time following `../incremental-implementation/SKILL.md` (`incremental-implementation`) and `../test-driven-development/SKILL.md` (`test-driven-development`). Use `context-engineering` to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
