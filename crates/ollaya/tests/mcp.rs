...
async fn client() -> rmcp::service::RunningService<rmcp::RoleClient, ()> {
    ...
    let transport = TokioChildProcess::new(tokio::process::Command::new(env!("CARGO_BIN_EXE_ollaya")).configure(|cmd| {
        cmd.arg("mcp")
...
async fn handshake_tools_and_resources() {
    ...
    assert!(info.instructions.as_deref().unwrap_or_default().contains("decide"));

    let mut tools: Vec<_> = client.list_all_tools().await.unwrap().into_iter().map(|t| t.name.to_string()).collect();
    tools.sort();
    assert_eq!(tools, ["decide", "list_models", "pull_model", "show_model"]);
    let decide = client.list_all_tools().await.unwrap().into_iter().find(|t| t.name == "decide").unwrap();
    ...
    for field in ["model", "state", "questions", "preset"] {
        assert!(schema["properties"][field].is_object(), "decide has no {field}: {schema}");
    }

    let uris: Vec<_> = client.list_all_resources().await.unwrap().into_iter().map(|r| r.uri.clone()).collect();
...
async fn bad_arguments_are_tool_errors() {
    ...
    let r = client.call_tool(call(json!({"state": "hi", "preset": "nope"}))).await.unwrap();
    assert_eq!(r.is_error, Some(true));
    let r = client
        .call_tool(call(json!({"state": "hi", "preset": "triage", "questions": {}})))
        .await
        .unwrap();
    assert_eq!(r.is_error, Some(true));
    client.cancel().await.unwrap();
